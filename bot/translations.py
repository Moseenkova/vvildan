...
WELCOME_MESSAGES = {
    ...
    "fr": (
        "Bonjour, {name} ! Cliquez sur le bouton du menu ou posez-moi directement "
        "votre question."
    ),
    ...
    "lt": (
        "Sveiki, {name}! Spustelėkite meniu mygtuką arba tiesiogiai užduokite man "
        "bet kokį klausimą."
    ),
    ...
    "sl": (
        "Pozdravljeni, {name}! Kliknite gumb menija ali mi neposredno zastavite "
    ...
    "uk": (
        "Привіт, {name}! Натисніть кнопку меню або поставте мені будь-яке запитання "
        "прямо в чаті."
    ),
    "ur": "سلام، {name}! مینو بٹن پر کلک کریں یا مجھ سے براہِ راست کوئی بھی سوال پوچھیں۔",
    "uz": (
        "Salom, {name}! Menyu tugmasini bosing yoki menga istalgan savolni "
